...
function formatHostEndpoints(status: DesktopStatus): string {
  const hosts = status.hostAddresses?.length ? status.hostAddresses : ["<this-pc-ip>"];
  return hosts.map((host) => `${host}:${status.port}`).join(" · ");
}
...
function TrackingView({ frame, status, model, onCalibrate, onEditMappings }: { frame: TrackingFrame; status: DesktopStatus; model: ImportedModel | null; onCalibrate(): void; onEditMappings(): void }) {
  ...
  return <div className="view-page">
    <section className={`tracking-hero${frame.faceFound ? " online" : ""}`}><div className="face-orbit"><div>⌁</div><i /></div><div><small>TRUEDEPTH LINK</small><h2>{frame.faceFound ? "Face tracking is live" : status.connectedDevices ? "iPhone connected — find your face" : "Connect your iPhone tracker"}</h2><p>{status.connectedDevices ? `${status.connectedDevices} secure tracker connection${status.connectedDevices === 1 ? "" : "s"} · frame ${frame.sequence || "idle"}` : <>Enter pairing code <strong>{status.pairingCode}</strong> in the LumaStage iPhone app. On Windows, auto-discovery often fails — use manual IP below.</>}</p></div><div className="tracking-actions"><button className="hero-action" disabled={!frame.faceFound} onClick={onCalibrate}>◎ Calibrate</button>{model && <button className="ghost-action" onClick={onEditMappings}>Edit mappings</button>}</div></section>
    ...
    <section className="privacy-strip"><div>♢</div><p><b>Manual / wired connect</b><br />If the phone cannot find this PC, type one of these hosts in the tracker app (same Wi‑Fi, Ethernet, or USB Personal Hotspot).</p><code>{formatHostEndpoints(status)}</code></section>
  </div>;
}
...
function SettingsView({ coreStatus, coreInstalling, status, onInstallCore, onForgetTrackers, onForgetPlugins }: { coreStatus: CubismCoreStatus; coreInstalling: boolean; status: DesktopStatus; onInstallCore(): void; onForgetTrackers(): void; onForgetPlugins(): void }) {
  ...
  return <div className="view-page">
    ...
    <section className="settings-grid">
      ...
      <article className="setting-card"><div className="setting-icon">⌁</div><div><small>IPHONE TRACKER</small><h3>Port {status.port}</h3><p>{status.connectedDevices} connected · {status.trustedDevices} paired device{status.trustedDevices === 1 ? "" : "s"}</p><p className="setting-hosts">Hosts: <code>{formatHostEndpoints(status)}</code></p><p className="setting-note">Windows: allow TCP {status.port} in firewall if manual connect fails. USB: enable iPhone Personal Hotspot over cable, then use the PC address shown here.</p>{status.trustedDevices > 0 && <button className="text-action" onClick={onForgetTrackers}>Forget paired devices</button>}</div></article>
...
function App() {
  ...
  const [status, setStatus] = useState<DesktopStatus>({ port: 39510, connectedDevices: 0, pairingCode: "------", trustedDevices: 0, vtsApiPort: 8001, vtsApiActive: false, allowedPlugins: 0, hostAddresses: [] });
    ...
    <section className="workspace">
      ...
      {activeView === "stage" && <div className="stage-grid">
        ...
        <aside className="inspector">
          ...
          <section className="panel hint"><div>⌁</div><p><b>Connect Tracker</b><br />Pairing code <strong>{status.pairingCode}</strong> on the iPhone. If auto-discovery fails (common on Windows), enter host manually:<br /><code>{formatHostEndpoints(status)}</code><br /><span className="hint-note">Wi‑Fi, Ethernet, or USB Personal Hotspot all work with manual IP. Allow TCP {status.port} in Windows Firewall if needed.</span>{status.trustedDevices > 0 && <button className="trust-reset" onClick={() => void window.lumastage.forgetTrustedDevices()}>Forget {status.trustedDevices} paired device{status.trustedDevices === 1 ? "" : "s"}</button>}</p></section>
