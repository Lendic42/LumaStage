...
function formatHostEndpoints(status: DesktopStatus): string {
  ...
  if (hosts.length === 1) return `${hosts[0]}:${status.port}`;
  // First address is the preferred LAN IP (tunnels/VPN sorted last on the desktop).
  const [primary, ...rest] = hosts;
  const extra = rest.length ? ` · also ${rest.map((host) => `${host}:${status.port}`).join(" · ")}` : "";
  return `use ${primary}:${status.port}${extra}`;
}
...
function TrackingView({ frame, status, model, onCalibrate, onEditMappings }: { frame: TrackingFrame; status: DesktopStatus; model: ImportedModel | null; onCalibrate(): void; onEditMappings(): void }) {
  ...
  return <div className="view-page">
    <section className={`tracking-hero${frame.faceFound ? " online" : ""}`}><div className="face-orbit"><div>⌁</div><i /></div><div><small>TRUEDEPTH LINK</small><h2>{frame.faceFound ? "Face tracking is live" : status.connectedDevices ? "iPhone connected — find your face" : "Connect your iPhone tracker"}</h2><p>{status.connectedDevices ? `${status.connectedDevices} secure tracker connection${status.connectedDevices === 1 ? "" : "s"} · frame ${frame.sequence || "idle"}` : <>Enter pairing code <strong>{status.pairingCode}</strong> in the LumaStage iPhone app. On Windows, Bonjour usually fails — open Manual IP on the iPhone and enter the primary LAN host (prefer 192.168.x, not VPN/Tailscale).</>}</p></div><div className="tracking-actions"><button className="hero-action" disabled={!frame.faceFound} onClick={onCalibrate}>◎ Calibrate</button>{model && <button className="ghost-action" onClick={onEditMappings}>Edit mappings</button>}</div></section>
    ...
    <section className="workspace">
      ...
      {activeView === "stage" && <div className="stage-grid">
        ...
        <aside className="inspector">
          ...
          <section className="panel hint"><div>⌁</div><p><b>Connect Tracker</b><br />Pairing code <strong>{status.pairingCode}</strong> on the iPhone. If auto-discovery fails (common on Windows), enter host manually:<br /><code>{formatHostEndpoints(status)}</code><br /><span className="hint-note">Use the first host IP on the same LAN. Skip 100.x/Tailscale and tunnel adapters unless both devices use that VPN. Set Windows network to Private and allow TCP {status.port}.</span>{status.trustedDevices > 0 && <button className="trust-reset" onClick={() => void window.lumastage.forgetTrustedDevices()}>Forget {status.trustedDevices} paired device{status.trustedDevices === 1 ? "" : "s"}</button>}</p></section>
